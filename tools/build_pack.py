#!/usr/bin/env python3
"""Build the Persian pack with the shared builder in vocab-engine.

Equivalent to:
    PYTHONPATH=engine/tools python3 -m packbuilder build --lang fa --repo . [--stage ...]

The pipeline lives in engine/tools/packbuilder (core stages) and
engine/tools/packbuilder/langs/fa.py (Persian rules). This repo keeps only the
data it owns: tools/gloss_overrides.json, tools/forced_a1.txt,
tools/generated_sentences.tsv, tools/id_map_v1.json, and the generated
pack/ + tools/REPORT.md.
...
if __name__ == "__main__":
    sys.exit(main(["build", "--lang", "fa", "--repo", str(ROOT)] + sys.argv[1:]))
